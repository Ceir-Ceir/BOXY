...
const NAV = [
  ...
  { href: "/thesis", label: "Thesis & Pitch", icon: Flame },
...
export default function Sidebar() {
  ...
  return (
    <>
      <QuickActionModal isOpen={quickOpen} onClose={() => setQuickOpen(false)} />
      ...
      <aside className="hidden md:flex w-[220px] shrink-0 flex-col border-r border-line bg-surface px-3 py-5 sticky top-0 h-screen">
        <Link href="/" className="flex items-center gap-2.5 px-3 mb-4">
        ...
        <button
          onClick={() => setQuickOpen(true)}
          className="btn btn-sm w-full justify-between mb-4 text-muted hover:text-ink border-line text-[12px] bg-raised"
        >
          <span className="flex items-center gap-1.5">
            <Search size={13} className="text-amber-ink" />
            Quick Action
          </span>
          <kbd className="num text-[10px] bg-surface px-1.5 py-0.5 rounded border border-line">
            ⌘K
          </kbd>
        </button>
        ...
        <div className="mt-auto px-1 pt-4 border-t border-line/60">
          <form action={logout}>
            <button className="btn btn-ghost w-full justify-start text-muted">
              <LogOut size={15} />
              Sign out
            </button>
          </form>
